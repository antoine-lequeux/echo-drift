...
CSprite::CSprite(GameObject& gameObject, std::shared_ptr<sf::Texture> tex, i32 drawOrder) : Component(gameObject), texture(tex), drawOrder(drawOrder)
{
...
{
    if (!texture) return {0.f, 0.f};

    if (textureRect.size.x == 0 || textureRect.size.y == 0) return {static_cast<f32>(texture->getSize().x), static_cast<f32>(texture->getSize().y)};
...
void CSprite::render(sf::RenderWindow& window)
{
    ...
    const sf::FloatRect viewBounds({viewCenter.x - viewSize.x / 2.f, viewCenter.y - viewSize.y / 2.f}, {viewSize.x, viewSize.y});
    ...
    const bool overlaps =
        spriteBounds.position.x + spriteBounds.size.x >= viewBounds.position.x && spriteBounds.position.x <= viewBounds.position.x + viewBounds.size.x &&
        spriteBounds.position.y + spriteBounds.size.y >= viewBounds.position.y && spriteBounds.position.y <= viewBounds.position.y + viewBounds.size.y;
    if (!overlaps) return;
